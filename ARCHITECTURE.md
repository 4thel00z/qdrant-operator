...
| `QdrantMigration` | One-shot copy from a managed cluster or any endpoint (URL + API key/CA secrets) into a managed cluster: scroll pages → upserts grouped by shard key; missing target collections are created from the source config with shard/replication counts left to the target unless overridden; payload indexes are re-created. Progress is patched every 20 batches. |
